...
- 构建一次：`pnpm -C viewport-kit-react build`
- 监听构建：`pnpm -C viewport-kit-react dev`
- 类型检查：`pnpm -C viewport-kit-react typecheck`
...
当你修改 `viewport-kit-react/src/**` 时，`tsup --watch` 会自动更新 `dist/`。多数消费端在刷新浏览器后会获取到变更。

### 方案 2：一次性构建

如果不想一直运行 watcher：

- 在 `viewport-kit-react`：`pnpm build`
...
- 在消费端创建一个 task，运行 `pnpm -C ../viewport-kit-react dev`
- 再创建一个 task，运行消费端的 `pnpm dev`
- 使用 compound task 同时运行两者。

备注：
- PowerShell 中用 `;` 链接命令。
- 建议保持 `viewport-kit-react` 的 watcher 在运行中，以便 `dist/` 持续更新。
...
1. 运行配置 A（库的 watch）：
   - package.json：`viewport-kit-react/package.json`
   - script：`dev`（tsup --watch）
   - 工作目录：`.../viewport-kit-react`
2. 运行配置 B（消费端）：
